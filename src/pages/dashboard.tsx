...
import { getBillingInfos } from 'apiSdk/billing-infos';
...
function DashboardPage() {
  ...
  const billingInfoStat = useSWR<PaginatedInterface<BillingInfoInterface>>(
    () => `/billing-infos`,
    () => getBillingInfos({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Billing Infos',
      result: billingInfoStat?.data?.totalCount,
      isLoading: billingInfoStat.isLoading,
    },
    {
      title: 'Rentals',
